...
@media print {
  ...
  static async generateExcel(
    ...
    const summaryData = [
      ['Executive Summary'],
      [''],
      ['Overall Health', llmAnalysis.choice.executiveSummary.overallHealth],
      [''],
      ['Key Highlights'],
      ...llmAnalysis.choice.executiveSummary.keyHighlights.map(h => [h]),
      [''],
      ['Critical Issues'],
      ...llmAnalysis.choice.executiveSummary.criticalIssues.map(i => [i]),
      [''],
      ['Immediate Actions'],
      ...llmAnalysis.choice.executiveSummary.immediateActions.map(a => [a])
    ];
    ...
    const metricsData = [
      ...
      ['Gross Margin', `${llmAnalysis.choice.financialPerformanceSnapshot.profitabilityAnalysis.margins.gross}%`],
      ['Operating Margin', `${llmAnalysis.choice.financialPerformanceSnapshot.profitabilityAnalysis.margins.operating}%`],
      ['Net Margin', `${llmAnalysis.choice.financialPerformanceSnapshot.profitabilityAnalysis.margins.net}%`],
      [''],
      ['Liquidity Ratios'],
      ['Current Ratio', llmAnalysis.choice.cashFlowAnalysis.liquidityPosition.currentRatio],
      ['Quick Ratio', llmAnalysis.choice.cashFlowAnalysis.liquidityPosition.quickRatio],
      ['Days of Cash on Hand', llmAnalysis.choice.cashFlowAnalysis.liquidityPosition.daysOfCashOnHand]
    ];
    ...
    const revenueData = [
      ...
      ...llmAnalysis.choice.revenueMetrics.topRevenueStreams.map(stream => [
        stream.source,
        stream.amount,
        `${stream.percentage}%`,
        `${stream.growth}%`
    ...
    const expenseData = [
      ['Expense Analysis'],
      [''],
      ['Category', 'Amount', '% of Revenue', 'Trend'],
      ...llmAnalysis.choice.expenseReview.majorExpenses.map(expense => [
        expense.category,
        expense.amount,
